...
public static class CM //Stands for Cache Memory!
{
    ...
    public static async Task<bool> Update<Ipaddress> (String key, Ipaddress value)
    {
        try
        {
            Console.WriteLine("[System]Trying to update value in cache...");
            if(cache != null)
            {
                if(!cache.TryGetValue(key, out _))
                {
                ...
                cache.Set(key, value, DefaultCacheOptions);
                Console.WriteLine("[System]Succes Update Value");
                return await Task.FromResult(true); //Success
            }
        }
        catch(Exception err)
        ...
        Console.WriteLine("[Error]Failed to update cache value");
        return false;
    ...
    {
        Console.WriteLine("[System]Trying to delete value from cache...");
        try
        {
            if(cache != null && key != null)
            {
                ...
                cache.Remove(key);
                Console.WriteLine("[System]Successfully deleted value");
                return await Task.FromResult(true); //Success
            }
        }
        catch(Exception err)
        {
            Console.WriteLine("[Error]Failed to delete value from cache");
            Console.WriteLine(err.Message);
        }
        return false;
    }
